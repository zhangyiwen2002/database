<template>
  <div class="box">
    <van-list
      ...
      @load="onLoad"
    >
      <van-swipe-cell v-for="item in allrecords.data" :key="item.id">
        <div class="whole" @click="toDetails(item.id)">
...
import { getUserRecords, deleteRecords } from "../../NetWork/article";
export default {
  ...
  methods: {
    ...
    onLoad() {
      setTimeout(() => {
        getUserRecords(0, store.state.id).then((res) => {
