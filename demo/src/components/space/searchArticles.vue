...
<script>
import { searchRecords } from "../../NetWork/article";
export default {
  name: "DemoSearchArticles",
  created() {
    console.log(this.value);
    this.allrecords.finished = false;
    this.onLoad();
  },
  beforeRouteUpdate(to, from, next) {
    ...
    this.allrecords.data.length = 0;
    this.allrecords.cnt = 0;
    this.allrecords.finished = false;
    this.onLoad();
  },
  data() {
  ...
  computed: {
    key() {
      return this.$route.name !== undefined
        ? this.$route.query.name + new Date()
        : this.$route + new Date();
    },
  },
  methods: {
    ...
    onLoad() {
      setTimeout(() => {
        searchRecords(this.value).then((res) => {
